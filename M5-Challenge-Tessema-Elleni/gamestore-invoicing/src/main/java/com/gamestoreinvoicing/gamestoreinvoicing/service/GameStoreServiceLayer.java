...
public class GameStoreServiceLayer {
    ...
    @Autowired
    public GameStoreServiceLayer(InvoiceRepository invoiceRepo, TaxRepository taxRepo, ProcessingFeeRepository processingFeeRepo, GameStoreFeignClient client) {
    ...
    public InvoiceViewModel createInvoice(InvoiceViewModel invoiceViewModel) {
        ...
        if (invoiceViewModel.getItemType().equals(CONSOLE_ITEM_TYPE)) {
            ...
            Optional<GameViewModel> returnVal = Optional.ofNullable(client.getGame(invoiceViewModel.getItemId()));
